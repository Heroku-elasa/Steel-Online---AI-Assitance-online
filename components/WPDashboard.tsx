      ...
      <div className="flex flex-1 overflow-hidden">
            ...
            {[
                ...
                <div key={idx} className={`${item.sep ? 'mt-3' : ''}`}>
                    ...
                    `}>
                        ...
                        {item.active && <div className={`absolute border-[8px] border-transparent top-1/2 -translate-y-1/2 ${isRTL ? 'border-r-[#f0f0f1] right-full mr-[-1px]' : 'border-l-[#f0f0f1] left-full ml-[-1px]'}`}></div>}
                    </div>
                </div>
            ))}
            
            {/* Collapse Menu */}
            <div className="mt-auto px-3 py-3 text-[#a0a5aa] hover:text-[#72aee6] cursor-pointer flex items-center gap-2 border-t border-[#2c3338]">
                <div className={`w-5 h-5 rounded-full border border-current flex items-center justify-center`}>
                    <svg className={`w-3 h-3 fill-current ${isRTL ? 'rotate-180' : ''}`} viewBox="0 0 20 20"><path fillRule="evenodd" d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z" clipRule="evenodd"/></svg>
                </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6">
            ...
            <div className="flex items-center justify-between mb-4">
                ...
                <button className="bg-white border border-[#c3c4c7] text-[#3c434a] px-3 py-1 text-[13px] rounded hover:border-[#8c8f94] hover:text-[#2271b1]">Screen Options ▼</button>
            </div>

            {/* Welcome Panel */}
            {showWelcome && (
                <div className="bg-white border border-[#c3c4c7] p-6 mb-6 relative">
                    <button onClick={() => setShowWelcome(false)} className="absolute top-4 right-4 text-[#787c82] hover:text-[#b32d2e] font-bold">✕</button>
                    <h2 className="text-[21px] font-normal mb-2 text-[#1d2327]">{t('wp_welcome_title')}</h2>
                    <p className="text-[16px] text-[#3c434a] mb-6">{t('wp_welcome_subtitle')}</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
                ...
                <div className="bg-white border border-[#c3c4c7] shadow-sm">
                    <div className="p-3 border-b border-[#dcdcde] flex justify-between items-center cursor-move">
                        <h3 className="font-semibold text-[14px]">{t('wp_at_a_glance')}</h3>
                        <button className="text-[#787c82]">▲</button>
                    </div>
                    <div className="p-4">
                        <div className="flex gap-4 mb-4 text-[#2271b1] text-[14px]">
            ...
            <div className="mt-8 border-t border-[#dcdcde] pt-3 text-[13px] text-[#646970] flex gap-4">
