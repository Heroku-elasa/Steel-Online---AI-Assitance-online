...
export const THEME_PRESETS: ColorScheme[] = [
    { id: 'industrial', name: 'Industrial Orange', primary: '#ea580c', secondary: '#0f172a' }, // Orange-600, Slate-900
    { id: 'steel', name: 'Steel Blue', primary: '#0284c7', secondary: '#0f172a' }, // Sky-600, Slate-900
    { id: 'construction', name: 'Construction Yellow', primary: '#ca8a04', secondary: '#1c1917' }, // Yellow-600, Stone-900
    { id: 'eco', name: 'Eco Green', primary: '#16a34a', secondary: '#064e3b' }, // Green-600, Emerald-900
];
...
export const AppearanceProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const [customLogo, setCustomLogo] = useState<string>(() => {
        return localStorage.getItem('dadgar-custom-logo') || "https://raw.githubusercontent.com/Heroku-elasa/steelonline20-Hour-Tash-Laboratory-Advanced-Analysis-Services-base-do-site/refs/heads/main/images.png";
    });
...
// --- TYPE DEFINITIONS ---
export type PageKey = 'legal_drafter' | 'lawyer_finder' | 'news_summarizer' | 'case_strategist' | 'notary_finder' | 'web_analyzer' | 'contract_analyzer' | 'evidence_analyzer' | 'image_generator' | 'corporate_services' | 'insurance_services' | 'site_architect' | 'court_assistant' | 'external_service' | 'general_questions' | 'blog' | 'content_hub' | 'pricing';
...
// Interface for a Supplier profile from the AI search or in saved state (replaces Lawyer)
export interface Lawyer {
    name: string;
    specialty: string; // Product Type (e.g. Rebar, Beam)
    city: string;
    contactInfo: string;
    address: string;
    website: string;
...
// Interface for a Warehouse/Logistics office (replaces Notary)
